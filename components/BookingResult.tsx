...
export default function BookingResult({ result }: BookingResultProps) {
  ...
  const handleDownloadPDF = () => {
    ...
    if (result.isBookingToday) {
      ...
      const days =
        result.daysUntilBooking;
  
      statusTitle =
        `Booking opens in ${days} ${
          days === 1 ? "day" : "days"
        }`;
    ...
    doc.save(
      `RailDate-${bookingDateText.replace(
        /[^a-zA-Z0-9]+/g,
        "-"
      )}.pdf`
    );
  ...
  return (
    ...
    text-transparent
    bg-clip-text
    bg-[linear-gradient(110deg,#818cf8_0%,#c084fc_22%,#22d3ee_50%,#c084fc_78%,#818cf8_100%)]
    bg-[length:250%_100%]
    animate-booking-date-flow
    drop-shadow-[0_0_18px_rgba(129,140,248,0.18)]
    sm:text-3xl
  "
            >
              {bookingDateText}
